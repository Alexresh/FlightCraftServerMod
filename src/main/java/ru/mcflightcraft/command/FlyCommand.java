...
public class FlyCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess commandRegistryAccess, CommandManager.RegistrationEnvironment registrationEnvironment) {
        dispatcher.register(CommandManager.literal("fly").requires(serverCommandSource -> serverCommandSource.hasPermissionLevel(4)).executes(FlyCommand::run));
    }
    ...
    private static int run(CommandContext<ServerCommandSource> serverCommandSourceCommandContext) throws CommandSyntaxException {
        ...
        if(serverPlayer.interactionManager.getGameMode() == GameMode.SURVIVAL){
            serverPlayer.getAbilities().allowFlying = !serverPlayer.getAbilities().allowFlying;
            serverPlayer.sendAbilitiesUpdate();
            serverPlayer.sendMessage(Text.literal("Fly: " + serverPlayer.getAbilities().allowFlying));
        }else {
